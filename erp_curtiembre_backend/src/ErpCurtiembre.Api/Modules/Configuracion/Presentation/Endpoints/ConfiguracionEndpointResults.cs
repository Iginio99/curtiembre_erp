...
internal static class ConfiguracionEndpointResults
{
    ...
    public static IResult From<T>(UseCaseResult<T> result)
    {
        ...
        {
            ConfiguracionErrorCodes.Validation => Results.BadRequest(new { error = result.ErrorCode, message = result.Message }),
            ConfiguracionErrorCodes.SessionRequired => Results.Json(new { error = result.ErrorCode, message = result.Message }, statusCode: StatusCodes.Status401Unauthorized),
            ConfiguracionErrorCodes.SessionExpired => Results.Json(new { error = result.ErrorCode, message = result.Message }, statusCode: StatusCodes.Status401Unauthorized),
            ConfiguracionErrorCodes.PermissionDenied => Results.Json(new { error = result.ErrorCode, message = result.Message }, statusCode: StatusCodes.Status403Forbidden),
            ConfiguracionErrorCodes.NotFound => Results.NotFound(new { error = result.ErrorCode, message = result.Message }),
            ConfiguracionErrorCodes.Conflict => Results.Conflict(new { error = result.ErrorCode, message = result.Message }),
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!sessionResult.Success || sessionResult.Data is null)
        {
            return (null, Results.Json(
                new
                {
                    error = sessionResult.ErrorCode ?? ConfiguracionErrorCodes.SessionRequired,
    ...
    public static async Task<(SessionInfoDto? Session, IResult? Failure)> RequireAdminAsync(
        ...
        await RequireAnyPermissionAsync(
            ...
            validateSessionUseCase,
            checkPermissionUseCase,
            [SeguridadPermissionCodes.UsuariosAdmin, SeguridadPermissionCodes.ConfiguracionAdmin],
            cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!allowed)
        {
            return (null, Results.Json(
                new
                {
                    error = ConfiguracionErrorCodes.PermissionDenied,
